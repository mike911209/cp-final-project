...
event_table = _dynamodb.Table(EVENT_TABLE)
...
class DecimalEncoder(json.JSONEncoder):
    def default(self, obj):
        if isinstance(obj, decimal.Decimal):
            # If it has no fractional part, return as int; otherwise float
            if obj % 1 == 0:
                return int(obj)
            else:
                return float(obj)
...
def lambda_handler(event, context):
    try:
        ...
        for item in items:
            alarm_resp = event_table.get_item(
                Key={'user_id': user_id, 'event_id': item['id']}
            )
            alarm = alarm_resp.get('Item')
            if alarm:
                for f in FIELDS:
                    if f in alarm:
                        item[f] = alarm[f]
        ...
        return {
            ...
            'body': json.dumps({'events': items}, cls=DecimalEncoder)
        }
